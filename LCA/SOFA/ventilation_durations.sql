...
CREATE OR REPLACE VIEW ventilation_durations AS
with vd0 as
(
  select
    icustay_id
...
(
  select
      ...
      , case
          ...
          when MechVent=1 then
            EXTRACT(EPOCH FROM (CHARTTIME - charttime_lag)) / 3600
          else null
        end as ventduration
...
select icustay_id
  ...
  , EXTRACT(EPOCH FROM (max(charttime) - min(charttime)))/3600 AS duration_hours
from vd2
